...
router.get('/users', authenticateToken, (req, res) => {
  // Return distinct user_id, username from listings where listing_type_id=1
  const query = `
  ...
  connection.query(query, (err, rows) => {
    ...
    // e.g. [ { user_id: 5, username: 'Alice' }, { user_id: 7, username: 'Bob' }, ... ]
    res.json(rows || []);
  });
});
 ...
 *   minPrice, maxPrice, type, search, users (CSV list of user IDs)
 */
router.get('/', authenticateToken, (req, res) => {
  ...
  // Parse "users" param as CSV, e.g. "1,2,3"
  if (users) {
...
router.post('/', authenticateToken, (req, res) => {
  ...
  // Adjust this constant if your "Marketplace" type ID is different.
 ...
 * These endpoints let users create, update, delete posts and comments for a marketplace listing.
 * They use post_type 'marketplace' and reference the listing via marketplace_id.
 */
...
router.get('/:listingId/posts/:postId', authenticateToken, (req, res) => {
  ...
  connection.query(query, [postId, listingId, userId, userId], (err, results) => {
    ...
    if (!results || results.length === 0) {
      return res.status(404).json({ error: 'Marketplace post not found' });
    }
    res.json(results[0]);
  });
});
...
/**
 * LIKE / UNLIKE for Marketplace Posts
 */
...
/**
 * LIKE / UNLIKE for Marketplace Comments
 */
