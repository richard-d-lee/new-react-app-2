...
router.delete('/:groupId/posts/:postId/like', authenticateToken, (req, res) => {
  ...
  const deleteQuery = 'DELETE FROM group_likes WHERE group_post_id = ? AND user_id = ?';
...
// POST /groups/:groupId/posts/:postId/comments - Create a comment on a group post
...
// DELETE /groups/:groupId/comments/:commentId/like
router.delete('/:groupId/comments/:commentId/like', authenticateToken, (req, res) => {
  const { groupId, commentId } = req.params;
  const userId = req.user.userId;
  const deleteQuery = `
    DELETE FROM group_comment_likes
  ...
  connection.query(deleteQuery, [commentId, userId], (err, results) => {
    ...
    res.json({ message: 'Group comment unliked successfully' });
  });
});

// POST /groups/:groupId/comments/:commentId/reply
router.post('/:groupId/comments/:commentId/reply', authenticateToken, (req, res) => {
  const { groupId, commentId } = req.params;
  const userId = req.user.userId;
  const { content, groupPostId } = req.body; // groupPostId must be sent by client
  if (!content || !content.trim()) {
    return res.status(400).json({ error: 'Content is required' });
  }
  const insertQuery = `
...
// POST /groups/:groupId/join - Join a group (groupId provided as URL parameter)
...
router.post('/:groupId/posts', authenticateToken, (req, res) => {
  ...
  connection.query(insertQuery, [groupId, userId, content], (err, insertResult) => {
    ...
    // 2) Now select the full post row (including username, profile_picture_url)
    const selectQuery = `
      SELECT 
    ...
    connection.query(selectQuery, [newGroupPostId], (err2, rows) => {
      ...
      // Return the newly created group post object
      res.json(rows[0]);
    });
  });
});
...
router.put('/:groupId/logo', authenticateToken, (req, res) => {
  // This endpoint expects that the file middleware (e.g., multer) has run.
  const groupId = req.params.groupId;
  if (!req.file) {
...
// POST /groups/:groupId/posts/:postId/like
...
router.post('/:groupId/comments/:commentId/reply', authenticateToken, (req, res) => {
  ...
  if (!groupPostId) {
    return res.status(400).json({ error: 'Group post ID (groupPostId) is required' });
  }
...
router.delete('/:groupId/comments/:commentId', authenticateToken, (req, res) => {
  ...
  // Adjust the table name and column as needed. Here we assume:
  // - The table for group comments is "group_comments"
  // - The primary key is "group_comment_id"
  const deleteQuery = `
...
// GET /groups/:groupId/posts/:postId/likes/count
